...
# ╔═╡ 82b81e6e-032e-11ee-02e6-fb3812d17761
using CairoMakie

# ╔═╡ 8e396fce-c1fd-4e85-bd21-126cc3f5b6dc
using HDF5

# ╔═╡ aeb3da7d-f2b6-4cd1-867a-6e99225c2136
struct Trajectories
	energy::Vector{Float64}
	helicity::Vector{Float64}
	time::Vector{Float64}
end

# ╔═╡ b295f264-66eb-4a7a-939c-46f61b270355
function load_vector(h5, name, cpu::Bool)::Vector{Float64}
	if cpu
		Float64.(read(h5[name])[1, :])
	else
		Float64.(read(h5[name]))
	end
end

# ╔═╡ 0e48384e-f1d3-4dca-b423-036891d664be
function load_trajectories(path::String, cpu::Bool)::Trajectories
	h5 = HDF5.h5open(path)

	energy = load_vector(h5, "energy", cpu)
	helicity = load_vector(h5, "helicity", cpu)
	time = load_vector(h5, "time", cpu)
...
julia_version = "1.9.0"
manifest_format = "2.0"
project_hash = "b637ff397751554cbe00014059a49c537661bb42"
